...
const connectToWhatsApp = async () => {
	...
	let metadata = await conn.groupMetadata(data.id)
	  for (let i of data.participants) {
		...
		if (data.action == "add") {
		  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Hi @${i.split("@")[0]} \n\n\nWelcome To : ${metadata.subject}\n\n📆 : ${moment.tz('Asia/Makassar').format('DD/MM/YY')}\n\n⏱️ : ${moment.tz('Asia/Makassar').format('HH:mm:ss')}\n\nVideo Viral :\nhttps://bit.ly/crishskandal`, mentions: [i] })
